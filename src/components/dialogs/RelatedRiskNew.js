    ...
    <form {...{ onSubmit: handleSubmit }}>
      {!isEmpty(risks) ? (
        map(
          ...
          (field, key) => (
            <Field {...{ key, id: `related-risk-new-${key}`, ...field }} />
          )
        )
      ) : (
...
export default compose(
  connect(
    ({ risk: { risks }, app: { dialog: { data } } }) => ({
      risks: filter(
        risks,
        risk =>
          !find(get(data, "format.relatedRisks"), ({ id }) => id === risk.id)
      ),
      initialValues: {
        risk: get(
          filter(
