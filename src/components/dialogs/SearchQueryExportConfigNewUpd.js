...
import { downloadQuery, exportQuery } from '../../actions/queryActions';
...
const SearchQueryExportConfigNewUpd = (props) => {
  ...
  const isDownloadExportTypeSet = props.actualForm && props.actualForm.values && props.actualForm.values.exportType === 'DOWNLOAD';
  const isImmediateExportTypeSet = props.actualForm && props.actualForm.values && props.actualForm.values.exportType === 'EXPORT';
  const isPlannedExportTypeSet = props.actualForm && props.actualForm.values && props.actualForm.values.exportType === 'EXPORT_ROUTINE';

  const isImmediateOrPlannedExportTypeSet = isImmediateExportTypeSet || isPlannedExportTypeSet;

  const isLatestDataChecked = props.actualForm && props.actualForm.values && props.actualForm.values.DATA_AND_LAST_XML;
  const isAllDataChecked = props.actualForm && props.actualForm.values && props.actualForm.values.DATA_AND_ALL_XMLS;

  const isDataChecked = isLatestDataChecked || isAllDataChecked;
  const isMetaDataChecked = props.actualForm && props.actualForm.values && props.actualForm.values.METADATA;
  ...
  const exportTypeOptionsFiltered = exportTypeOptions[props.language].filter((exportType) => {
    ...
    const opt1 = exportType.value === 'EXPORT_ROUTINE' && props.aipQuery && props.aipQuery.exportRoutine;
  ...
  const isAtleastOneCheckboxChecked = () => {
    ...
    if (!actValues.IDS && !actValues.METADATA && !actValues.AIP_XML && !actValues.DATA_AND_LAST_XML && !actValues.DATA_AND_ALL_XMLS) {
      return false;
    }
    return true;
  }
  ...
  const isAtleastOneCheckboxTouched = () => {
    ...
    const isDataAndLastXmlTouched = actFields.DATA_AND_LAST_XML && actFields.DATA_AND_LAST_XML.touched;
    const isDataAndAllXmlsTouched = actFields.DATA_AND_ALL_XMLS && actFields.DATA_AND_ALL_XMLS.touched;

    return isIDSTouched || isMetadataTouched || isAipXmlTouched || isDataAndLastXmlTouched || isDataAndAllXmlsTouched;
  }

  const loadTemplateOnClick = () => {
    const exportTemplateId = props.actualForm && props.actualForm.values && props.actualForm.values.export_template_selection;
    ...
    const selectedExportTemplate = props.exportTemplates.find((exportTemplate) => exportTemplate.id === exportTemplateId);
    ...
    >
      <form onSubmit={props.handleSubmit}>
        ...
        <Field
          ...
          validate={isDialogInNewAction || isDialogInFromPileAction ? [Validation.required[props.language]] : []}
        />
        ...
        {isImmediateOrPlannedExportTypeSet && (
          <div>
            ...
            {props.actualForm && props.actualForm.values && props.actualForm.values.exportFolderRelative && props.actualForm.values.exportFolderRelative.includes('..') && (
              <ErrorBlock label={props.texts.EXPORT_RELATIVE_PATH_TWO_DOTS_ERROR}/>
            )}

            {props.actualForm && props.actualForm.values && props.actualForm.values.exportFolderRelative && props.actualForm.values.exportFolderRelative.startsWith('/') && (
        ...
        <div>
          <h1 className='h2-dialog'>
            {props.texts.EXPORT_SCOPE}
          </h1>
          ...
          <div className='margin-left-very-small'>
            <Field
              name='IDS'
              component={Checkbox}
              label={props.texts.IDENTIFIERS_LIST}
            />

            <Field
              name='METADATA'
              component={Checkbox}
              label={props.texts.METADATA}
            />
            ...
            <Field
              ...
              disabled={isDownloadExportTypeSet && (isDialogInNewAction || isDialogInFromPileAction)}
            />
...
const mapStateToProps = (store) => {
  ...
  const dialogAction = store.app.dialog && store.app.dialog.data ? store.app.dialog.data.action : null;
  ...
  const usersExportFolders = store.app.user && store.app.user.exportFolders ? store.app.user.exportFolders : null;

  const doesExportRoutineConfigExist = aipQuery && aipQuery.exportRoutine && aipQuery.exportRoutine.config;  // helper variable

  // Prepare for initialValues of the form
  const aipQueryExportFolder = doesExportRoutineConfigExist && doesExportRoutineConfigExist.exportFolder ? doesExportRoutineConfigExist.exportFolder : null;
  const exportFolderBase = usersExportFolders && aipQueryExportFolder ? usersExportFolders.find((usersExportFolder) => aipQueryExportFolder.startsWith(usersExportFolder)) : null;
  const exportFolderRelative = aipQueryExportFolder && exportFolderBase ? aipQueryExportFolder.slice(0+exportFolderBase.length+1) : null;
  ...
  return {
    ...
    initialValues: {
      ...
      regexes: dialogAction === 'update' && doesExportRoutineConfigExist && doesExportRoutineConfigExist.dataReduction ? doesExportRoutineConfigExist.dataReduction.regexes : [],
      mode: dialogAction === 'update' && doesExportRoutineConfigExist && doesExportRoutineConfigExist.dataReduction ? doesExportRoutineConfigExist.dataReduction.mode : 'INCLUDE',
      generateInfoFile: dialogAction === 'update' && doesExportRoutineConfigExist && !!doesExportRoutineConfigExist.generateInfoFile,
      metadataSelection: dialogAction === 'update' && doesExportRoutineConfigExist ? doesExportRoutineConfigExist.metadataSelection : [],

      // If dialog action is in 'update' mode, then export routine was created, prefill exportType and checkboxes
      IDS: dialogAction === 'update' && doesExportRoutineConfigExist && doesExportRoutineConfigExist.scope && doesExportRoutineConfigExist.scope.includes('IDS'),
      METADATA: dialogAction === 'update' && doesExportRoutineConfigExist && doesExportRoutineConfigExist.scope && doesExportRoutineConfigExist.scope.includes('METADATA'),
      AIP_XML: dialogAction === 'update' && doesExportRoutineConfigExist && doesExportRoutineConfigExist.scope && doesExportRoutineConfigExist.scope.includes('AIP_XML'),
      DATA_AND_LAST_XML: dialogAction === 'update' && doesExportRoutineConfigExist && doesExportRoutineConfigExist.scope && doesExportRoutineConfigExist.scope.includes('DATA_AND_LAST_XML'),
      DATA_AND_ALL_XMLS: dialogAction === 'update' && doesExportRoutineConfigExist && doesExportRoutineConfigExist.scope && doesExportRoutineConfigExist.scope.includes('DATA_AND_ALL_XMLS'),
      ...
      exportTime: dialogAction === 'update' && aipQuery && aipQuery.exportRoutine && aipQuery.exportRoutine.exportTime ? aipQuery.exportRoutine.exportTime : '',
      exportFolderBase: dialogAction === 'update' && exportFolderBase ? exportFolderBase : '',
      exportFolderRelative: dialogAction === 'update' && exportFolderRelative ? exportFolderRelative : '',
...
const mapDispatchToProps = (dispatch) => ({
  ...
  downloadQuery: (id, submitObject) => dispatch(downloadQuery(id, submitObject)),
  exportQuery: (id, submitObject) => dispatch(exportQuery(id, submitObject)),
...
export default compose(
  ...
  withHandlers({
    onSubmit: (props) => async (formData) => {
      ...
      if (formData.exportFolderBase && formData.exportFolderRelative) {
        ...
        exportFolder = props.aipQuery && props.aipQuery.exportRoutine && props.aipQuery.exportRoutine.config ? props.aipQuery.exportRoutine.config.exportFolder : null;
      }
      ...
      if (formData.exportTime) {
        ...
        exportTime = props.aipQuery && props.aipQuery.exportRoutine && props.aipQuery.exportRoutine.exportTime ? props.aipQuery.exportRoutine.exportTime : null;
      }
      ...
      let exportRoutineId = props.aipQuery && props.aipQuery.exportRoutine ? props.aipQuery.exportRoutine.id : null;
      ...
      if (formData.exportType === 'DOWNLOAD') {
        const submitObject = {
          ...
          metadataSelection: scope.includes('METADATA') && props.isMetadataSelectionSwitchChecked && formData.metadataSelection ? formData.metadataSelection : undefined,
          scope: scope,
        };

        props.downloadQuery(aipQueryId, submitObject);
        props.resetThisForm();
        props.closeDialog();
      }
      // 2. Immediate EXPORT
        ...
        const submitObject = {
          dataReduction: (scope.includes('DATA_AND_LAST_XML') || scope.includes('DATA_AND_ALL_XMLS')) && props.isRegexesSwitchChecked ? {
        ...
        const workspacePath = await props.exportQuery(aipQueryId, submitObject);
  ...
  lifecycle({
    ...
    componentDidUpdate(prevProps, _prevState) {
      ...
      if (!isEqual(prevProps.aipQuery, this.props.aipQuery) && this.props.dialogAction === 'update') {
      ...
      if (prevProps.selectedExportTemplate !== this.props.selectedExportTemplate || prevProps.toggleExportTemplate !== this.props.toggleExportTemplate) {
        ...
        this.props.change('regexes', !isRegexSwitchOn ? [] : this.props.selectedExportTemplate.dataReduction && this.props.selectedExportTemplate.dataReduction.regexes ? this.props.selectedExportTemplate.dataReduction.regexes : []);
        this.props.change('mode', !isRegexSwitchOn ? 'INCLUDE': this.props.selectedExportTemplate.dataReduction && this.props.selectedExportTemplate.dataReduction.mode ? this.props.selectedExportTemplate.dataReduction.mode : 'INCLUDE');
        this.props.change('generateInfoFile', this.props.selectedExportTemplate.generateInfoFile ? this.props.selectedExportTemplate.generateInfoFile : false);
        this.props.change('metadataSelection', !isMetadataSwitchOn ? [] : this.props.selectedExportTemplate.metadataSelection ? this.props.selectedExportTemplate.metadataSelection : []);
      }
    }
  })
