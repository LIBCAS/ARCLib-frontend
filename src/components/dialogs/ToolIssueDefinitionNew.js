...
const ToolIssueDefinitionNew = ({ handleSubmit, texts, language, issueDictionary, data }) => (
  <DialogContainer
    {...{
    ...
    <form {...{ onSubmit: handleSubmit }}>
      {!isEmpty(issueDictionary) ? (
        map(
          ...
          (field, key) => <Field {...{ key, id: `tool-issue-definition-new-${key}`, ...field }} />
        )
      ) : (
...
export default compose(
  connect(
    ({
      ...
      issueDictionary: filter(
        issueDictionary,
        (issue) => !find(get(data, 'tool.possibleIssues'), ({ id }) => id === issue.id)
      ),
      initialValues: {
        issue: get(
          filter(
  ...
  withRouter,
  withHandlers({
    onSubmit: ({ closeDialog, texts, data, putTool, issueDictionary, reset }) => async ({
      issue,
    }) => {
      if (
        await putTool({
          ...
          possibleIssues: get(data, 'tool.possibleIssues')
            ? [...data.tool.possibleIssues, find(issueDictionary, ({ id }) => id === issue)]
