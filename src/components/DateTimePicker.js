...
import { compose, lifecycle, withState, withProps, defaultProps } from 'recompose';
...
const onPlaceholderChange = (index, number, placeholder, language, timeFormat) => {
  ...
  if (dateTimePicker) {
    ...
    if (inputNode) {
      inputNode.setAttribute('placeholder', hasValue(placeholder) ? placeholder : '');
      inputNode.setAttribute('id', `datetimepicker-component-input-${index}-${number}`);
      ...
      inputNode.onmouseleave = () =>
        document
          .getElementById(`datetimepicker-component-input-${index}-${number}`)
          .setAttribute('placeholder', hasValue(placeholder) ? placeholder : '');
    }
  }
};
