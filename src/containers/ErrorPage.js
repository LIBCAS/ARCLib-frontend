        ...
        <div>
          ...
          {hasValue(get(responseStatusWithErrorMessage, "message")) && (
            <div {...{ className: "error-page-text" }}>
              {map(
                get(responseStatusWithErrorMessage, "message", "").split("\n"),
                (part, i) => (
                  <span {...{ key: i }}>
                    {i > 0 && <br />}
                    {part}
...
export default compose(
  withRouter,
  withState(
    "responseStatusWithErrorMessage",
  ...
  lifecycle({
    componentWillMount() {
      ...
      try {
        const responseStatusWithErrorMessage = JSON.parse(
          storage.get("error-page-content")
        );
