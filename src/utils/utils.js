...
export const removeStartEndWhiteSpaceInSelectedFields = (objectValue, fields) => {
  const modifiedObject = objectValue;

  forEach(fields, (field) => {
    if (get(objectValue, field)) {
      set(modifiedObject, field, removeStartEndWhiteSpace(get(objectValue, field)));
    }
  });
  ...
  if (userInfo) {
    ...
    if (hasPermissions()) {
      [
        ...
        [Permission.STORAGE_ADMINISTRATION_READ, '/archival-storage-administration'],
